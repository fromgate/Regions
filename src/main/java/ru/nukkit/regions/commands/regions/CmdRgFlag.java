...
@CmdDefine(command = "region", alias = "rg", subCommands = {"flag|f", "\\S+", "\\S+", "\\S+"}, defaultPerm = Permission.DEFAULT_TRUE, allowConsole = true, permission = "regions.flag", description = Message.RG_FLAG_DESC)
public class CmdRgFlag extends Cmd {
    ...
    public boolean execute(CommandSender sender, Player player, String[] args) {
        ...
        if (flag == null || !Regions.getManager().addFlag(args[1], flag))
            return Message.RG_FLAG_FAIL.print(sender, args[2].toUpperCase());
